...
function getName(x) {
  // const name = x.name;
  return 'Hello, my name is ' + x.name;
}
...
function makeSmartPerson(name) {
  return nameObject = {name:name,sum:(a,b)=>a+b,speak:()=>'Hello, my name is ' + name};

  // UNSIMPLIFIED VERSION BELOW IN CASE PREFERRED
  // const nameObject = {
  //   name: name,
  //   sum: (a,b) => a + b,
...
function getModelYears(inventory) {
  // return [inventory[num].car_year for num in inventory]
  let years = [];
  for ( x in inventory ){
    years.push(inventory[x].car_year);
  }
  return years;
}
...
function carMaker(num) {
  return carObject = {odometer:num,drive:(distance)=>carObject.odometer = carObject.odometer + distance}
}
